...
API key hardening (resolves V4 deferred item): docker-compose.yml now sources DEFAULT_API_KEY and MASTER_API_KEY via env-var substitution with dev fallbacks (${DEFAULT_API_KEY:-dev-local-siem-key-2026}, ${MASTER_API_KEY:-master-key-change-in-prod}) instead of hardcoded plaintext values. .env.example added as a committed template; real .env remains gitignored.

docs/demo-script.md created: 5-minute narrated demo script covering dashboard overview, log viewer, live attack simulation (Run Demo), case management, and incident report generation.

README updated: Docker Hub pull instructions (Option A: clone+build, Option B: pull pre-built images), documented amd64-only decision and Rosetta workaround for Apple Silicon.

QA verification (fresh Docker state):
- Full clean rebuild (docker system prune -a --volumes -f then docker compose up --build): ~115 seconds total, well under the 5-minute target
- Full pytest suite: 68/68 passing, 0 warnings
- Lighthouse Accessibility: 82/100 (target 70+) - 2 Critical issues remain (missing label on filter dropdowns, insufficient contrast on secondary/muted text in theme.js) - logged as optional follow-up polish, not blocking
- Firefox cross-browser render: confirmed visually identical to Chrome

Deferred / open items carried forward:
- V5 stretch goal not attempted this version (second demo scenario / screen-capture video / read-only embed view) - candidate for future polish, not blocking V5 close-out
- V4 stretch goal (scheduled reports / report comparison / RBAC) still open from V4 - candidate for a future version
- 2 Lighthouse Critical accessibility issues noted above - optional follow-up
